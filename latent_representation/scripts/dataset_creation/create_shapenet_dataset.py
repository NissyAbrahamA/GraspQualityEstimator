...
def create_scenes_centered_fixed(scenes_dir, lib, scene_size=0.297):
    """
        object will not be rotated
        object will be placed in the center of the scene
        if object too large, will not create a scene for that object
    """
    split_fn = os.path.join(scenes_dir, f'all.lst')
    with open(split_fn, 'w') as _:
        # this is just to clear the file
        pass

    for k, (identifier, shape) in enumerate(lib.items()):
        print(f'{k}/{len(lib)}: {identifier} ({shape.name})')

        aabb = shape.mesh.get_axis_aligned_bounding_box()
        extents = aabb.get_extent()
        if np.any(extents > scene_size):
            print(f'shape too large, cannot fit into scene without rescaling.\n\t'
                  f'extents: {extents}. scene size: {scene_size}')
            continue

        target_center = np.ones(3) * scene_size/2 - aabb.get_center()
        target_pose = np.eye(4)
        target_pose[:3, 3] = target_center

        # now we can load object mesh with correct scale and planned orientation
        instance = burg.ObjectInstance(shape, target_pose)
        ...
        with open(split_fn, 'a') as split_file:
            split_file.write(f'{identifier}\n')

    print('done creating scenes.')
...
def main():
    ...
    dataset_dir = 'c:/users/rudorfem/datasets/gag-refine/'
    shapenet_dir = 'c:/users/rudorfem/datasets/ShapeNetCore.v2/ShapeNetCore.v2/'
    library_dir = 'object_library/object_library.yaml'
    library_dir = 'object_library/object_library_watertight.yaml'
    ...
    lib = burg.ObjectLibrary.from_yaml(os.path.join(dataset_dir, library_dir))
    print(f'got object library with {len(lib)} items')

    # creates meshes and updates
    # manifold.make_meshes_watertight(lib, target_dir='watertight_meshes')
    ...
    # ALTERNATIVE WAY OF CREATING SCENES
    # create_scenes_centered_fixed(scenes_dir, lib)
    ...
    # dataset_creation.print_occupancy_summary(scenes_dir, splits=splits)
