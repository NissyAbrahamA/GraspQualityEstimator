...
def get_latent_code(args):
    ...
    for it, data in enumerate(test_loader):
        ...
        #print(data)
        points_tensor = data['points']
        pre_norm_points = pre_normalise_points(points_tensor)
        data['points'] = pre_norm_points
        latent_code = backbone.eval_scene_pc_for_latent_code(data)
        #print('latent_code' + str(latent_code))
        np.save('002_master_chef_can', latent_code)
