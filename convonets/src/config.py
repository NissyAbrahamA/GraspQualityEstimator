...
from convonets.src import data
from convonets.src import conv_onet, conv_sdfnet
from .checkpoints import CheckpointIO


method_dict = {
    'conv_onet': conv_onet,
    'conv_sdfnet': conv_sdfnet,
}

# set default config fn path so other modules can retrieve it
default_config_fn = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../configs/default.yaml')
...
def get_dataset(mode, cfg, return_idx=False):
    ...
    if dataset_type == 'Shapes3D':
        ...
        fields['meta'] = data.MetaDataField()
...
def get_model_interface(cfg, device, dataset):
    model = get_model(cfg, device, dataset)
    convonet_checkpoint_io = CheckpointIO(cfg['training']['out_dir'], model=model)
    convonet_checkpoint_io.load(cfg['test']['model_file'])
    return method_dict[cfg['method']].config.get_model_interface(model, device, cfg)
